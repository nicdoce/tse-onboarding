...
export function TaskDetail() {
  ...
  const assigneeName = task?.assignee ? task.assignee.name : "Not assigned";
    ...
    <Page>
      <div className={styles.pageContent}>
        ...
        <div className={styles.titleContainer}>
          ...
          {task && <Button className={styles.button} label="Edit task" />}
        </div>
        {task && <p className={styles.taskDescription}>{taskDescription}</p>}
        <div className={styles.container}>
          {task && <p className={styles.item}>Assignee</p>}
          {task && assigneeName && <img src={task.assignee?.profilePictureURL}></img>}
          {task && <p>{assigneeName}</p>}
        </div>
        <div className={styles.container}>
